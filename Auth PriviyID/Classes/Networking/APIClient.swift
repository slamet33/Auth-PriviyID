...
class APIClient {
    ...
    func requests<T: Codable> (endPoint: APIConfiguration) -> Single<ApiResult<T, ErrorResponse>> {
        
        return Single<ApiResult<T, ErrorResponse>>.create { single in
    ...
    static func upload<T: Codable>(endPoint: APIConfiguration, file: Data?, parameters: [String: String]?, fileType: FileUploadExtension) -> Observable<ApiResult<T, ErrorResponse>> {
        
        return Observable<ApiResult<T, ErrorResponse>>.create({ observer in
            ...
            .responseData { response in
                ...
                if 200..<300 ~= httpResponseStatusCode {
                    ...
                    do {
                        let model = try JSONDecoder().decode(ErrorResponse.self, from: responseData)
