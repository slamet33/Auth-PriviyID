...
//  tenantapp
//
//  Created by Raja Syahmudin on 27/06/20.
//  Copyright © 2020 RoomMe. All rights reserved.
//

import UIKit

public protocol SwiftToastViewProtocol: AnyObject {
    func nib() -> SwiftToastViewProtocol?
    func configure(with toast: SwiftToastProtocol)
}

public enum CustomSwiftAlertType {
    case error
    case success
    case warning
}
...
class RMToastView: UIView, SwiftToastViewProtocol {
    ...
    func nib() -> SwiftToastViewProtocol? {
        return Bundle.main.loadNibNamed("RMToastView", owner: self, options: nil)?.first as? RMToastView
    }
    ...
    func configure(with toast: SwiftToastProtocol) {
        if let customToast = toast as? CustomSwiftToast {
            ...
            titleLabel.textColor = customToast.textColor
            subtitleLabel.text = customToast.subtitle
            subtitleLabel.textColor = customToast.textColor
            containerView.backgroundColor = customToast.backgroundColor
            containerView.layer.borderColor = customToast.borderColor.cgColor
            bottomConstraint.constant = customToast.bottomConstraint
        }
    }
}
